...
// Edwin vd Oetelaar, december 2024, okt 2025
#include "bsp.h"
...
static void ssd1306_write_cmd (ssd1306_handle_t *handle, uint8_t cmd) {
    ...
    data[1] = cmd;
    ed_err_t ret = bsp_i2c_master_transmit (handle->device_address, data, sizeof (data));  // i2c_master_transmit(*(handle->dev_handle), data, sizeof(data), pdMS_TO_TICKS(1000));

    if (ret != ED_OK) {
        printf ("Error writing command 0x%02X", cmd);
    }
}

#define MAX_CHUNK_SIZE 32  // Maximum size of each data chunk

static ed_err_t write_data_chunk (ssd1306_handle_t *handle, const uint8_t *data, size_t chunk_size) {
    ...
    return bsp_i2c_master_transmit (handle->device_address, buf, chunk_size + 1);
}
...
static void ssd1306_write_data (ssd1306_handle_t *handle, const uint8_t *data, size_t length) {
    ...
    while (remaining_length > 0) {
        ...
        ed_err_t ret = write_data_chunk (handle, data + written, chunk_size);
        if (ret != ED_OK) {
...
void ssd1306_init (ssd1306_handle_t *dev, uint8_t width, uint8_t height, uint8_t external_vcc) {
    ...
    ssd1306_show (dev);
    printf ("SSD1306 initialized, W=%d, H=%d", width, height);
}
...
static void draw_character (ssd1306_handle_t *dev,
...
uint8_t ssd1306_printFixed8 (ssd1306_handle_t *dev, uint8_t xpos, uint8_t ypos, uint8_t color, const char *str) {
    ...
    for (size_t i = 0; i < str_len; i++) {
        ...
        if (font_table_index < 0 || font_table_index >= (signed)ARRAY_SIZE (font_petme128_8x8)) {
